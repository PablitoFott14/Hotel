/* ═══════════════════════════════════════════
   calendar.js — Check-In / Check-Out events
   UTC date keys verified vs reservation dates
═══════════════════════════════════════════ */
...
const AP_CAL_TAG_COLOR = {
  'Check-In':  'var(--azure)',
  'Check-Out': 'var(--emerald)',
};
function _calTagColor(tag) { return AP_CAL_TAG_COLOR[tag] || 'var(--violet)'; }

function _calDateKey(ts) { return new Date((ts > 1e12 ? ts : ts * 1000)).toISOString().slice(0, 10); }
function _calMonthOf(ts) { return _calDateKey(ts).slice(0, 7); }
function _calShiftMs(ms, delta) {
...
function _calGridKeys(monthStr) {
  ...
  const keys = [];
  for (let i = 0; i < 42; i++) {
    keys.push(new Date(Date.UTC(y, m - 1, 1 - dow + i)).toISOString().slice(0, 10));
  }
  return keys;
}
function _calFmtMonth(ms) {
  const [y, m] = ms.split('-').map(Number);
...
function renderCalendar(DATA) {
  ...
  _calAll = [...events].map((e, i) => ({ ...e, _idx: i })).sort((a, b) => a.start_datetime - b.start_datetime);
  ...
  document.getElementById('calendar-content').innerHTML = `
    <div class="cal-filter-bar">
      <input id="cal-search" class="ex-search" type="text" placeholder="Guest name, room, confirmation…" />
  ...
  document.getElementById('cal-search').addEventListener('input', e => { _calSearch = e.target.value.toLowerCase(); _calMonth = ''; _calSelKey = ''; _drawCal(); });
  document.getElementById('cal-tag').addEventListener('change', e => { _calTag = e.target.value; _calMonth = ''; _calSelKey = ''; _drawCal(); });
  _drawCal();
}
...
function _drawCal() {
  ...
  if (!filtered.length) { document.getElementById('cal-body').innerHTML = '<div class="empty-state">No events match.</div>'; return; }
  ...
  if (!_calSelKey || !gridKeys.includes(_calSelKey)) _calSelKey = withItems[0] || (_calMonth + '-01');

  const selectedItems = grouped[_calSelKey] || [];

  let html = `<div class="cal-shell">
    <div class="cal-nav">
      <button class="cal-nav-btn" onclick="_apCalShift(-1)">‹</button>
  ...
  gridKeys.forEach(key => {
    ...
    html += `<button type="button" class="cal-cell${!sameMo?' other-month':''}${isSel?' selected':''}" onclick="_apCalSelectDay('${key}')">
  ...
  html += `</div>
    <div class="cal-agenda">
      <div class="cal-agenda-hdr">${_calFmtKey(_calSelKey)} · ${selectedItems.length} event${selectedItems.length!==1?'s':''}</div>`;
  ...
  selectedItems.forEach(e => {
    ...
    html += `<div class="cal-agenda-item" onclick="showCalEventModal(${e._idx})">
      ...
      <div style="flex:1">
        <div class="cal-agenda-title">${escHtml(e.title||'—')}</div>
      ...
      <span class="badge" style="flex-shrink:0;background:${col}22;color:${col}">${escHtml(e.tag||'—')}</span>
    </div>`;
  });
...
function _apCalShift(delta) { _calMonth = _calShiftMs(_calMonth, delta); _calSelKey = ''; _drawCal(); }
function _apCalSelectDay(key) { _calSelKey = key; _drawCal(); }
